...
`schemen-credential-broker`, version **0.1.1**, requiring Python **3.11+** on
...
exists. The pinned dependency snapshot includes the test runner. Install the
...
All endpoints except health require `Authorization: Bearer <broker-token>`.
...
provider responses to 1 MiB. Redirects and compressed provider responses are
refused. Upstream cookies and headers are not forwarded. Common direct/encoded
credential echoes cause an error instead of returning the body.
...
The [security contract](SECURITY.md) maps claims to adversarial tests. Test
