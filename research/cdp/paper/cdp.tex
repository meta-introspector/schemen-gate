...
\begin{enumerate}[nosep]
    ...
    inventory contains 21 modules and 317 active theorem/lemma declarations
    with no active \texttt{sorry} or \texttt{admit} declarations.  Four
    project-specific \texttt{axiom} declarations remain: two opaque
    predicates and two historical statistical consequences. The former
    global recovery bound now requires an explicit, unvalidated premise.  The core Hadamard and aligned FFN confinement
...
The mathematical gate is one element-wise product over $d$ coordinates per
gated tensor per invocation.  The checked-in fail-closed runtime evaluates the
finite-value-equivalent operation as a Boolean selection, writing positive zero
on excluded coordinates so that IEEE-754 NaN and infinity do not survive there.
It requires $d$ element-wise selections with no accumulation in the forward
pass and the corresponding selection in the backward pass, against
...
element-wise selection, its memory traffic is two vector reads and one vector
write and may be
...
gate in this study: its systems figures are
\Cref{sec:deployment}'s state-dict arithmetic and
\Cref{sec:permutation-equivariance}'s keyed-basis benchmark, and the
historical throughput figures are withheld pending hardware-matched
rerun (\Cref{sec:deployment}).  A later library-level NumPy component
benchmark, added after this study, measures the fail-closed public API and is
indexed in \path{docs/PERFORMANCE.md}; it is not model-level or submission
...
The checked-in proof inventory contains 21 Lean modules and 317 active
...
that thirteen selected algebraic, placement, output-validity, and
independence theorems use only
Lean/Mathlib's standard logical axioms.

The source tree separately contains four project-specific
\texttt{axiom} declarations. Two declare opaque predicates:
\begin{itemize}[nosep]
    \item \texttt{Recovers} (opaque predicate)
    \item \texttt{IsDistributionMatched} (opaque predicate)
\end{itemize}
They are not premises of the audited supported theorems. The former global
recovery-cost axiom has been removed. Historical wrappers now require an explicit
\texttt{FullEnumerationAssumption}, for which no instance is supplied.
Candidate cardinality does not establish recovery work or key
entropy. The softmax lemma establishes a valid real-valued distribution, not
confidence, correctness, or statistical concealment. The selected-output and
generation independence results instead compare states that agree on the
active surface while fixing the specified public functions and inputs.
...
Selected-coordinate output locality and independence under fixed active
values, projection, and bias. Historical weight/partition recovery chains
are excluded from the supported claim set \\
