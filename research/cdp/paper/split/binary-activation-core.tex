...
gated tensor. The checked-in fail-closed runtime evaluates the
finite-value-equivalent operation as $d$ Boolean selections with no
accumulation and writes positive zero on excluded coordinates so that IEEE-754
NaN and infinity do not survive there. Writing the operation as
...
selection is $\Theta(d)$ against the
...
the per-request selection, but it does not instantiate the declared
...
more implementation-dependent storage. A later library-level NumPy component
benchmark is indexed in \path{docs/PERFORMANCE.md}; it postdates this study and
is not model-level evidence, so this paper makes no numerical latency claim.
...
The accompanying pinned Lean 4 project builds 21 modules and 317 active theorem or
lemma declarations, with no active \texttt{sorry} or \texttt{admit}
declarations. A checked-in \texttt{\#print axioms} audit shows that thirteen
selected gate, placement, cardinality, and output statements use only
Lean/Mathlib's standard logical axioms. Four project-specific \texttt{axiom}
declarations are present
in separate conditional or historical theorem chains; none reaches the
headline statements used here.

The historical recovery-cost chain now takes an explicit, unvalidated
\texttt{FullEnumerationAssumption} as a caller premise. The size of a support
space does not establish attack cost or a pseudorandom-function reduction.
Likewise, softmax normalization establishes a valid probability distribution,
not answer correctness, confidence, or concealment of an incorrect regime.
